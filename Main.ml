...
type tipo = 
  ...
  | TyRef of tipo            
  | TyUnit         
  | TyList of TyInt         (* lista de inteiros *)          
...
let rec typeInfer (e:expr) : tipo option =  
  ...
  | If (e1, e2, e3) -> 
    ...
    | Some TyBool -> 
      (match (typeInfer e2, typeInfer e3) with
        | (Some t2, Some t3) -> if t2 = t3 then Some t2 else None
        | _ -> None)
      | _ -> None)
  ...
  | Asg (e1, e2) -> 
    ...
    | Some TyRef t1 -> 
      (match typeInfer e2 with
      | Some t2 -> if t1 = t2 then Some TyUnit else None
      | _ -> None)
    | _ -> None)

  | Let (string, tipo, e1, e2) -> 
    (match typeInfer e1 with
    | Some t1 ->  if t1 = tipo then                  
                    (match typeInfer e2 with
                    | Some t2 -> Some t2
                    | _ -> None)
                  else None
  ...
  | Cons (e1, e2) -> 
    (match (typeInfer e1, typeInfer e2) with
    | (Some TyInt, Some TyList) -> Some TyList
    | _ -> None)
  ...
  | Suffix (e1, e2) ->  (* l.n *)
    (match (typeInfer e1, typeInfer e2) with
    | (Some TyList, Some TyInt) -> Some TyList
    | _ -> None)
...
let rec step (e:expr) : expr option =  
  ...
  | Binop (bop, Num v1, e2) -> 
      (match step e2 with
      | Some e2' -> Some (Binop (bop, Num v1, e2'))
      | None -> None)
  | Binop (bop, e1, e2) -> 
    (match step e1 with
